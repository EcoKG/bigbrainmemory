...
- [ ] **T34. 자동 부호화 넛지** (T31·T33 이후, A/B 필수)
  - 사람은 의도 없이 부호화한다(Craik & Lockhart 1972). BBM 의 생성 진입점은 `remember()` 하나뿐이고 `src/types.ts` 에 현저성·트리거 필드가 없다(`salience`/`surprise`/`novelty` 소스 전체 0건).
  - **자동 저장 경로는 만들지 않는다** — 훅은 큐만 내고 판단은 모델이 한다(P6 자동삭제 금지의 대칭). 큐는 자신이 기계 휴리스틱임을 밝히고 출처·타입을 하드코딩하지 않는다(출처혼동 방지).

- [ ] **T35. 공고화 — episodic → semantic** (설계 확정, 착수 보류)
  - `reflect()` 는 위생점검만 하고, `consolidate` 프롬프트는 이름만 공고화이지 7단계 전부가 청소다. `type` 은 `store.ts` 에서 한 번 설정된 뒤 어디서도 대입되지 않고 `ReviseInput` 에 `type` 이 없어 **오분류조차 교정 불가**다.
  - **공고화 3조(P8 긴장의 처리)**: ① 바이트 불변 — 파생물 생성이 기존 파일을 1바이트도 바꾸지 않음(볼트 전체 SHA-256 맵으로 회귀 검사) ② 출처 영속 — `derivedFrom` 을 frontmatter 에 저장하고 본문 헤더는 거기서 재생성(revise 가 본문을 갈아도 출처 유지) ③ 인출면 우선순위 — 파생물이 자기 근거를 RIF 로 누르지 않음.
