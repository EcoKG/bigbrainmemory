...
- [ ] **T33. 사용 계측** (다음 단계, blocking)
  - 저장률·회상률·오염률을 **재는 수단이 코드에 없다.** A/B 하네스는 커밋돼 있지 않고 기억으로만 존재한다.
  - `access_count` 는 지연 플러시 때문에 회상 빈도 지표로 못 쓴다(F-1). 실험 기간에는 `BIGBRAIN_FLUSH_EVERY_ACCESS=1` 또는 append-only 사용 로그(JSONL)가 필요하다.
  - **천장효과 주의**: 훅 ON 저장률이 29/30 이라 그 조건에서 "세션당 저장>0" 을 1차 지표로 쓰면 팔당 30회로 어떤 개선도 탐지 못 한다. 1차 지표를 **"트리거 발생 턴 → 저장까지 걸린 턴 수"** 같은 연속량으로 두거나 양팔 훅 OFF 로 고정해야 한다.
