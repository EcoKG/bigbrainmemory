...
- [ ] **T28. 이관 제목 복원 2패스** (N3 — medium)
  - 무엇: 이관 시 제목이 `toast-notify-deliverable` 로 들어간다. 사람이 읽는 `ToastNotify deliverable` 은 네이티브 MEMORY.md 인덱스에만 있고 노트의 `name:` 은 슬러그다. 주입 채널이 제목을 보여주므로 이관 후 인덱스 가독성이 통째로 떨어진다.
  - 왜 지금 안 하나: **의도된 절충을 뒤집는 변경**이다 — T20 이 "제목을 원본 `name` 그대로 써 본문 `[[위키링크]]` 보존" 을 명시적으로 선택했다. BBM 은 slug 를 title 에서 파생하므로 제목을 바꾸면 노트 간 링크가 전부 깨진다. 사용자 기억 데이터를 만지는 스크립트라 단독 커밋으로 신중히 해야 한다.
  - 어떻게: 2패스 — 1패스에서 `name → 최종 slug` 매핑 수집, 2패스에서 본문 `[[name]]` 을 새 slug 로 치환 후 저장. 제목은 네이티브 MEMORY.md 의 `- [제목](파일.md)` 에서 복원하고 없으면 `fm.name` 폴백.
  - 검증: 실제 코퍼스 23건 `--apply` 후 `reflect` 의 `broken_links` 가 0건. 미리보기가 치환 결과를 먼저 보여줄 것.

- [ ] **T29. 이관 project 스코프 정규화** (N4 — medium)
  - 무엇: `project=C--Users-ruinp-OneDrive-------Discord-CLI-bot` — 뭉개진 경로 슬러그가 그대로 들어간다. `BIGBRAIN_PROJECT` 를 이 문자열과 정확히 같게 써야 스코프가 맞는데 사람이 손으로 쓸 값이 아니고, 프로젝트를 옮기면 조용히 어긋난다.
  - 어떻게: 마지막 의미 구간을 프로젝트명으로 정규화(`Discord-CLI-bot`) + `--project-map <원본>=<이름>` 으로 사용자 덮어쓰기. 원본 슬러그는 `source` 에 이미 남으므로 추적성 무손실.
  - 검증: 정규화 결과가 충돌하면(두 경로가 같은 이름) 미리보기에서 경고하고 중단.
